...
```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-channel-v1.git --from 0.0.0-preview
```

Then add `GoogleCloudChannelV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudChannelV1 <target-name> --package swift-google-cloud-channel-v1
```
